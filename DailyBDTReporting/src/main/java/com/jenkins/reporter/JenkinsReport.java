...
public class JenkinsReport {
	...
	public List<CIBuild> buildCIObject(String API, String value) {
		try {
			...
			for (Element buildElement : buildElements) {
				...
				if (lastBuildNumber >= Integer.parseInt(ciBuild.getBuildNumber())) {
					...
					for (Element subBuildElement : subBuildElements) {
						...
						if (subBuildElement.selectSingleNode("result") == null)
							subBuild.setSubBuildStatus("ABORTED");
						else
			...
			log.debug("buildCIObject :" + e);
			System.out.println("buildCIObject : Somthing is wrong");
			return null;
		}
	}
	...
	private String getJobString(String x, String api) {
		try {
			...
			if ("http://192.168.5.252:8090/job/iContract_BDTD/api/xml".equals(api))
				...
				return x + job;
			else if ("http://192.168.5.252:8090/job/iAnalyze_BDTD/api/xml".equals(api))
				return x + job;
		} catch (Exception e) {
			log.debug("getJobString :" + e);
			System.out.println("getJobString : Somthing is wrong");
		}
		return api;
	}
