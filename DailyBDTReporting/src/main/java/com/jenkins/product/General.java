				...
				for (SubBuild subBuild : subBuilds) {
					...
					if ((productName + BUILD).equals(subBuild.getSubBuildJobName())) {
						if (HibernateWrappers
								...
								.isEmpty()) {
							finalReport.setBuildRepo(NO_VALUE);
							...
							build_run_details run_details = HibernateWrappers
									.getObjectListByRestrictionPostgreSQL(
											...
											build_run_details.class)
									.get(0);
							finalReport.setBuildRepo(run_details.getRepo_url());
							finalReport.setBuildBranch(run_details.getBranch());
						}
						...
						if (subBuild.getSubBuildStatus().equals("ABORTED"))
					...
					finalReport.setTesting_type(NO_VALUE);
				...
				for (SubBuild subBuild : subBuilds) {
					if ((productName + BUILD).equals(subBuild.getSubBuildJobName())) {
						...
						if (HibernateWrappers
							...
							finalReport.setTesting_type(testDetailsSUB.getTesting_type());
						}
