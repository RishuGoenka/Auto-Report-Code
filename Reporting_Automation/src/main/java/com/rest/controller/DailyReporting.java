...
public class DailyReporting {
	...
	public static void main(String[] args) {
		try {
			...
			System.out.println("Daily Report  : Somthing is wrong");
		}
	}
		...
		try {
			...
			for (String prod : products) {
				if (!dailyreportdata.containsKey(prod))
					dailyreportdata.put(prod, new DailyReport("Running", "-", "-", "-", "-", "-", "-"));
			}

			for (Map.Entry<String, DailyReport> entry : dailyreportdata.entrySet()) {
				String product = entry.getKey();
				DailyReport report = entry.getValue();
				mailBody += "<tr>" + "<td style=\"border:1px solid #000\"><font color=\"#1B1BEE\"> " + product
						+ " </font></td>" + "<td style=\"border:1px solid #000\">" + report.getBuildStatus() + "</td>"
